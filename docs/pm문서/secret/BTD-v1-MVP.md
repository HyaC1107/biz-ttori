...
- 🔵 **npm registry 실배포 — 보류 결정 (2026-07-20 PM 판단)**: "완성도 문제로 아무나 npx 설치하게 두긴 이르다" — IP 보호가 아니라 폴리시 미달 우려. **결론: 포폴 목적은 GitHub 레포 자체로 충분, 실제 publish는 불필요.** 배포 준비 트랙(패키징/번들 전략)은 여기서 정지.
  - **젬또리 리서치 원문검증 완료(2026-07-20, WebFetch)** — 2건 정정됨:
    - ✅→⚠️정정 **패키지명 `btd` 선점 확인**(사실)이나 젬또리가 근거를 지어냄("12년전 Buffer charset info"는 오답) — **실제는 2022-04-20 등록된 "2진수↔10진수 변환" 유틸(v1.0.1)**, `registry.npmjs.org/btd` 원문 확인. 결론(이름 못 씀)은 유효 → 대안 `@hyac1107/btd` 또는 `btd-cli`.
    - ❌ **2FA 필수화 주장은 과장/오답** — 젬또리는 "2FA 없으면 배포 거부"라 했으나 공식문서(`docs.npmjs.com/generating-provenance-statements`)엔 **선택적 보안기능**이라고만 돼있음(강제 아님). provenance 권장이라는 부분만 정확.
    - ③ `create-vite`류 단일번들+`files`필드 동봉 관례는 **미검증**(낮은 우선순위라 스킵, 재개 시 확인).
