# stack-fullstack.md — [보관됨, 2026-07-27] 구 React Native/Fullstack 기술 스택 트러블슈팅 아카이브

> ⚠️ **보관 문서.** 업무 중심이 AI 모델 개발로 전환되며 `memory/stack.md`에서 이곳으로 이동했다. 실증 트러블슈팅 지식은 가치가 있어 삭제하지 않고 보존한다. 현재 유효한 표준 스택은 `memory/stack.md`를 참고.

이 문서는 **Biz-Ttori** 워크스페이스에서 진행되었던 풀스택, React, React Native 프로젝트들의 주요 기술 스택과 환경 설정, 그리고 자주 발생하는 트러블슈팅 솔루션을 기록합니다.
